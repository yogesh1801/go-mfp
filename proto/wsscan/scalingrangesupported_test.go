...
func TestScalingRangeSupported_XMLRoundTrip(t *testing.T) {
	...
	for _, c := range cases {
		srs := ScalingRangeSupported{
			ScalingWidth:  ScalingWidth{MinValue: MinValue(c.wmin), MaxValue: MaxValue(c.wmax)},
			ScalingHeight: ScalingHeight{MinValue: MinValue(c.hmin), MaxValue: MaxValue(c.hmax)},
		}
...
func TestScalingRangeSupported_Validation(t *testing.T) {
	badCases := []ScalingRangeSupported{
		{
			ScalingWidth:  ScalingWidth{MinValue: 0, MaxValue: 100},
			ScalingHeight: ScalingHeight{MinValue: 100, MaxValue: 200},
		},
		{
			ScalingWidth:  ScalingWidth{MinValue: 100, MaxValue: 200},
			ScalingHeight: ScalingHeight{MinValue: 0, MaxValue: 100},
		},
		{
			ScalingWidth:  ScalingWidth{MinValue: 100, MaxValue: 99},
			ScalingHeight: ScalingHeight{MinValue: 100, MaxValue: 200},
		},
		{
			ScalingWidth:  ScalingWidth{MinValue: 100, MaxValue: 200},
			ScalingHeight: ScalingHeight{MinValue: 100, MaxValue: 99},
		},
	}
