...
//
// ValWithBoolAttr: reusable type for elements with
...
// ValWithBoolAttr holds a value and optional boolean attributes.
...
type ValWithBoolAttr[T any] struct {
	Text        T
...
// decodeValWithBoolAttr fills the struct from an XML element.
// The decoder function converts the XML text to the desired type T.
func (t *ValWithBoolAttr[T]) decodeValWithBoolAttr(
	root xmldoc.Element,
	decoder func(string) (T, error),
) (ValWithBoolAttr[T], error) {
...
func (t ValWithBoolAttr[T]) toXML(
	name string,
